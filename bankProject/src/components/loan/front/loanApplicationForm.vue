            ...
            <form @submit.prevent="handleSubmit" enctype="multipart/form-data">
                <div class="form-grid">
                    ...
                    <div class="form-item">
                        ...
                        <input type="text" v-model="form.memberName" id="memberName" readonly />
                    </div>
                    ...
                    <div class="form-item">
                        ...
                        <div class="input-with-unit">
                            <input type="number" v-model="form.loanAmount" id="loanAmount" min="1" max="500"
                                placeholder="最多 500 萬元" required />
                            <span>萬</span>
                        </div>
                    </div>
                    ...
                    <div class="form-item">
                        ...
                        <div class="custom-select-wrapper">
                            <select v-model="form.loanTypeId" id="loanTypeId" @change="fetchInterestRate" required>
                    ...
                    <div class="form-item">
                        ...
                        <div class="custom-select-wrapper">
                            <select v-model="form.loanTermId" id="loanTermId" @change="handleTermChange" required>
                        ...
                        <div class="custom-select-wrapper">
                            <select v-model="form.accountId" id="accountId" required>
                                ...
                                <option v-for="acc in accounts" :key="acc.accountId" :value="acc.accountId">
                                    {{ acc.accountName }} ({{ acc.currency }} {{ formatAccountBalance(acc.balance) }})
                                </option>
                            </select>
                        </div>
                        <!-- 顯示帳戶載入狀態 -->
                        <div v-if="accountsLoading" class="loading-text">載入帳戶資料中...</div>
                        <div v-else-if="accounts.length === 0" class="no-accounts-text">尚未找到可用帳戶</div>
                    </div>
                    ...
                    <div class="form-item interest-display">
                        <label>預估利率：<span class="rate-value">{{ interestRateDisplay }}</span></label>
                    </div>

                    <!-- 月付款試算 -->
                    <div class="form-item payment-display" v-if="monthlyPayment > 0">
                        <label>預估月付款：<span class="payment-value">NT$ {{ formatNumber(monthlyPayment) }} 元</span></label>
                    </div>
                </div>
...
const fetchInterestRate = async () => {
    ...
    try {
        ...
        // 2. 如果有貸款金額，計算月付款
        if (form.value.loanAmount && form.value.loanAmount > 0 && form.value.loanTerm) {
            try {
...
// 監聽貸款金額變更
watch(() => form.value.loanAmount, () => {
    if (form.value.loanTypeId && form.value.loanTermId && form.value.loanAmount) {
        fetchInterestRate();
    }
});
