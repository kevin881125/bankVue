<template>
  <div class="container">
    <h2>工作者操作紀錄</h2>
    <div class="searchbar">
      <div class="input"><input type="text" v-model="searchData" placeholder="請輸入帳號" /><button class="serchbtn" @click="search">查詢</button></div>
    ...
    <!-- <table> 
      ...
      <tbody>
        <tr v-for="worlerLog in worlerLogs">
          <td>{{ worlerLog.worker.wId }}</td>
          <td>{{ worlerLog.worker.wAccount }}</td>
          <td>{{ worlerLog.worker.wName }}</td>
          <td>{{ worlerLog.action }}</td>
          <td>{{ worlerLog.message }}</td>
          <td>{{ worlerLog.time }}</td>
        </tr>
      </tbody>
    </table> -->
    <div class="table">
...
const search =async ()=>{
    const data = await request({
    url: "/workerlog/account/"+searchData.value,
    method: "GET",
  });
  worlerLogs.value = data;
}
