<template>
    <div class="homepage">
        <!-- 頂部導航區 - 僅登入後顯示 -->
        <header v-show="memberStore.isLoggedIn" class="top-header" :class="{ 'header-animate-enter': isAnimating }">
        ...
        <!-- 主視覺區域 -->
        <section class="hero-section" :class="{ 'with-header': memberStore.isLoggedIn }">
            <div class="hero-content">
                <!-- 左側標語區 -->
                <div class="hero-text">
                    <div v-show="!memberStore.isLoggedIn" class="hero-logo" :class="{ 'logo-animate-up': isAnimating }">
                        <img src="../../image/logo_horizontal.png" alt="柚子銀行" />
                    </div>
                    ...
                    <p class="hero-subtitle" :class="{ 'subtitle-animate-up': isAnimating }">
                        | 柚子銀行 | 最瞭解您的專業財務夥伴
                    </p>
                    <div class="hero-buttons" :class="{ 'button-animate-fade': isAnimating }">
        ...
        <!-- 功能卡片區域 -->
        <section class="services-section" :class="{ 'services-with-header': memberStore.isLoggedIn }">
            <div class="services-container">
                <div class="service-cards">
                    <!-- 我的帳戶 -->
                    <div class="service-card" :class="{ 'card-animate-enter': isAnimating }">
                    ...
                    <!-- 信用卡 -->
                    <div class="service-card multi-option-card" :class="{ 'card-animate-enter': isAnimating }"
                        @mouseenter="hoveredCard = 'credit'" @mouseleave="hoveredCard = null">
                        ...
                        <div v-show="hoveredCard === 'credit'" class="options-list">
                            <router-link to="/yuzubank/cardShowcase" class="option-item">信用卡申請</router-link>
                            <router-link to="/yuzubank/record" class="option-item">申請紀錄</router-link>
                            <router-link to="/yuzubank/myCard" class="option-item">我的卡片</router-link>
                            <router-link to="/yuzubank/creditTransaction" class="option-item">交易紀錄</router-link>
                            <router-link to="/yuzubank/myBill" class="option-item">我的帳單</router-link>
                        </div>
                    </div>
                    ...
                    <!-- 基金 -->
                    <div class="service-card multi-option-card" :class="{ 'card-animate-enter': isAnimating }"
                        @mouseenter="hoveredCard = 'fund'" @mouseleave="hoveredCard = null">
                                ...
                                <div class="icon-circle">
                                    <svg viewBox="0 0 24 24" fill="none">
                                        <path d="M3 17L9 11L13 15L21 7" stroke="currentColor" stroke-width="2"
                                            fill="none" />
                                        <path d="M21 7H15V1" stroke="currentColor" stroke-width="2" fill="none" />
                                    </svg>
                                </div>
                            </div>
                            <h3>基金</h3>
                        ...
                        <div v-show="hoveredCard === 'fund'" class="options-list">
                            <router-link to="/yuzubank/fundHome" class="option-item">基金總覽</router-link>
                            <router-link to="/yuzubank/fundPortfolio" class="option-item">投資組合</router-link>
                            <router-link to="/yuzubank/fundMarket" class="option-item">市場行情</router-link>
                        </div>
                    </div>
                </div>
            </div>
        ...
        <!-- 登出確認視窗 -->
        <LogoutModal :visible="showLogoutModal" message="您確定要登出嗎？" @confirm="handleLogout"
...
// 監聽登入狀態變化
watch(() => memberStore.isLoggedIn, (newValue, oldValue) => {
